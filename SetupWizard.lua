...
function Wizard:CreateFrame()
    ...
    captureInput:SetScript("OnTabPressed", function(self)
        Wizard:OnKeyDown("TAB")
    end)
    ...
    frame.buttonGrid = buttonGrid
    frame.movementTitle = movementTitle
    self.frame = frame
...
function Wizard:BuildSteps()
    ...
    for slotIndex = firstActionSlot, actionCount do
        self.steps[#self.steps + 1] = {
            kind = "action",
            slotIndex = slotIndex,
            title = combatLabels[slotIndex] or ("Action " .. slotIndex),
            text = "Press the button for " .. (combatLabels[slotIndex] or ("Action " .. slotIndex)) .. ". Bare press becomes a normal action slot, and each modifier gives it a matching page slot.",
        }
    end
end
...
function Wizard:StoreKey(step, key)
    ...
    elseif step.kind == "action" then
        GPX:SetSetupActionKey(self.state, step.slotIndex, key)
    end
end
...
function Wizard:FormatMovementSummary()
    ...
    local modifiers = self.state.modifiers or {}
    local labels = {
        GPX:GetConfirmLabel(self.state.deviceId) .. ": " .. (self.state.jumpKey or "--"),
...
function Wizard:RefreshSummary()
    ...
    local style = GPX:GetInputStyle(self.state.deviceId)
    local actionCount = GPX:GetConfiguredActionButtonCount(self.state)
    local firstActionSlot = GPX:GetActionKeyBaseSlot(self.state)
    local combatLabels = GPX:GetCombatSlotLabels(self.state.deviceId)
    local rows = math.max(1, math.ceil(actionCount / 4))
    ...
    for cardIndex, card in ipairs(self.frame.actionCards) do
        if cardIndex <= actionCount then
            local slotIndex = firstActionSlot + cardIndex - 1
            card.label:SetText(combatLabels[slotIndex] or ("Action " .. slotIndex))
            card.value:SetText(GPX:GetSetupActionKey(self.state, slotIndex) or "Press to assign")
            card:Show()
        else
            card:Hide()
        end
    ...
    if self.frame.buttonGrid then
        self.frame.buttonGrid:SetHeight((rows * 92) - 12)
    end
    if self.frame.movementTitle and self.frame.buttonGrid then
        self.frame.movementTitle:ClearAllPoints()
        self.frame.movementTitle:SetPoint("TOPLEFT", self.frame.buttonGrid, "BOTTOMLEFT", 0, -12)
    end
...
function Wizard:Refresh()
    ...
    self.frame.captureHint:SetText("ESC pauses capture. TAB is accepted. Duplicate keys are rejected so the layout stays deterministic.")
    self.frame.applyButton:Disable()
    self.frame.startButton:Disable()
    self.frame.skipButton:Disable()
end
...
function Wizard:BuildSetupPayload()
    return {
        ...
        actionKeyBaseSlot = self.state.actionKeyBaseSlot,
        actionButtonCount = self.state.actionButtonCount,
    }
end
...
function Wizard:Apply()
    ...
    local firstActionSlot = GPX:GetActionKeyBaseSlot(self.state)
    local lastActionSlot = GPX:GetConfiguredActionButtonCount(self.state)
    if not self.state.jumpKey then
        GPX:Print("Setup is incomplete. Capture jump / confirm first.")
        return
    end

    for slotIndex = firstActionSlot, lastActionSlot do
        if not GPX:GetSetupActionKey(self.state, slotIndex) then
            GPX:Print("Setup is incomplete. Capture all configured combat buttons before applying.")
            return
        end
    end

    if not self.state.modifiers or #self.state.modifiers < 3 then
        GPX:Print("Setup is incomplete. Capture all three modifiers before applying.")
        return
    end
