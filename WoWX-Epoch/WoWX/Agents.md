...
- The direct resolved-slot fallback for base `ACTIONBUTTONn` buttons is only used when the client is both:
  - reporting the fake stale override state
  - and still reporting a live bonus page
- This reduced the delayed recovery where base keys could stay pinned to the old Runeshroud slot for too long after leaving the form.
