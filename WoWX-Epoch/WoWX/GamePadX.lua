...
function GPX:AppendActionStateTrace(trigger)
    if not self.db then
        return
    end

    self.db.stateTrace = self.db.stateTrace or {}

    local override = HasOverrideActionBar and HasOverrideActionBar() and 1 or 0
    local bonus = HasBonusActionBar and HasBonusActionBar() and 1 or 0
    local bonusOffset = GetBonusBarOffset and (tonumber(GetBonusBarOffset()) or 0) or 0
    local form = GetShapeshiftForm and (tonumber(GetShapeshiftForm()) or 0) or 0
    local page = GetActionBarPage and (tonumber(GetActionBarPage()) or 0) or 0
    local suspended = self.actionStateSuspended and 1 or 0
    local reason = self.actionStateReason or ""
    local line = string.format(
        "%s %s override=%d bonus=%d bonusOffset=%d form=%d page=%d suspended=%d reason=%s",
        date("%H:%M:%S"),
        tostring(trigger or "event"),
        override,
        bonus,
        bonusOffset,
        form,
    ...
    table.insert(self.db.stateTrace, line)
    while #self.db.stateTrace > 50 do
        table.remove(self.db.stateTrace, 1)
    end
end

function GPX:PrintActionStateTrace(limit)
    local trace = self.db and self.db.stateTrace or nil
    if not trace or #trace == 0 then
        self:Print("No saved trace entries yet.")
        return
    end

    local count = tonumber(limit) or 12
    if count < 1 then
        count = 1
    end
    local startIndex = math.max(1, (#trace - count) + 1)
    self:Print("Trace entries: showing " .. tostring(#trace - startIndex + 1) .. " of " .. tostring(#trace))
    for index = startIndex, #trace do
        self:Print(trace[index])
    end
end
...
function GPX:Slash(msg)
    ...
    elseif cmd == "trace" then
        local arg = string.lower((rest or ""):match("^%s*(.-)%s*$"))
        if arg == "" or arg == "show" or arg == "status" then
            self:PrintActionStateTrace()
        elseif arg:match("^show%s+%d+$") then
            local count = tonumber(arg:match("show%s+(%d+)$"))
            self:PrintActionStateTrace(count)
        elseif arg == "clear" then
            self.db.stateTrace = {}
            self:Print("Action-state trace cleared.")
        elseif arg == "mark" then
            self:AppendActionStateTrace("manual_mark")
            self:Print("Action-state trace mark captured.")
        else
            self:Print("Usage: /wowx trace [show|show N|clear|mark]")
        end
...
mainFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        ensureInitialized()
        GPX:RefreshActionStateSafety(true)
        GPX:AppendActionStateTrace("PLAYER_ENTERING_WORLD")
    ...
    elseif event == "PLAYER_REGEN_ENABLED" then
        ...
        GPX:RefreshActionStateSafety(true)
        GPX:AppendActionStateTrace("PLAYER_REGEN_ENABLED")
    elseif event == "UNIT_ENTERED_VEHICLE" or event == "UNIT_EXITED_VEHICLE" then
        local unit = ...
        if unit == "player" then
            GPX:RefreshActionStateSafety()
            GPX:AppendActionStateTrace(event)
        end
